...
Reporting opens only an existing, closed host-local database through SQLite's immutable read-only URI mode with query-only enforcement. It creates no state directory or database, performs no migration or metadata write, and refuses a snapshot with live SQLite sidecars or any table missing a required current-schema column. After an accepted milestone review, the stored Run and milestone-specific history select milestone reporting even if the plan projection row is missing. Current acceptance survives only when one canonical stored plan has its recomputed plan and contract digests, complete validated Task topology, and exact native Task, worker, gate, packet, preflight, candidate, source, and workspace bindings; missing or conflicting history is stale evidence until exact restoration. A missing, unreadable, malformed, or reparse-blocked current profile has the same fail-closed result.
