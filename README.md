...
Reporting opens only an existing, closed host-local database through SQLite's immutable read-only URI mode with query-only enforcement. It creates no state directory or database, performs no migration or metadata write, and refuses a schema that needs migration or a snapshot with live SQLite sidecars. After an accepted milestone review, reporting selects the stored Run before reading the current profile; a missing, unreadable, malformed, or reparse-blocked profile therefore retracts current acceptance as stale evidence until the exact identity is restored. Candidate reconstruction accepts only canonical v3 packets that pass the normal packet validator and agree across their exact native Task, Dispatch, candidate, contract, source, and workspace bindings.
