...
| Synthetic machine-bootstrap regressions | The same complete suite under POSIX and simulated-Win32 providers: `.cmd` forms, case-insensitive PATH spelling, file-index-style identity, typed registry state, target-parent byte-exact commits and EXDEV fallback, staged-versus-committed digest equality, continuous commit-to-pip archive binding, physically bound installation and external-anchor ancestry, nested-descendant-bound venv creation, proof-preserving receipt diagnostics, concurrency, and fault seams; the native API wrapper remains Windows-only | No matching correction-candidate record yet | `NOT_RUN` |
...
Unit coverage injects a memory-only typed user-PATH store, transactional registry adapter, synthetic command resolver and subprocess runner, and disposable installation/source/state roots. A shared high-level provider contract selects bounded reads, physical pins, target-parent binary staging, and exact commit verification; native Windows and the simulated provider use different OS primitives at that boundary. The complete unit and incident corpus runs under both the POSIX provider and simulated-Win32 provider; the latter covers `.cmd` forms, case-insensitive spelling, stable identity, registry semantics, and share-binding analogues without touching a real profile. Regressions prove that every staged commit preserves bare LF bytes and equal staged/committed digests, injected newline drift fails, an injected anonymous-link `EXDEV` takes the same-parent named fallback, and pip receives the descriptor continuously retained from the reviewed archive commit. Transient archive input, staged-name substitutions, and post-commit/pre-pin public-path replacement cannot become authority; redirected installation ancestry is refused before repair mutation; redirected anchor ancestry is refused; the venv's nested pip destination is pinned before the builder runs; receipt wrapping preserves proof-grade identity fields; and the documented local-venv and bytecode exclusions are exact. The native wrapper remains separately authoritative for CreateFile sharing, explicit application-name execution, registry transactions, real junctions, and hosted Windows behavior. The suite cannot prove live registry permissions, environment propagation, `py.exe` discovery, pip/network behavior, WSL Windows-PATH import, or mounted-file execution; those remain live `NOT_RUN` gates.
