...
| Synthetic machine-bootstrap regressions | Fake typed/CAS user-PATH store and resolver, disposable physically bound venv/install roots, bounded fd-pinned receipt reads, exclusive random staging, event-backed concurrent invocations, synthetic subprocesses, and spelling-robust path faults; no live registry, PATH, or Python mutation | No matching correction-candidate record yet | `NOT_RUN` |
...
Unit coverage injects a memory-only typed user-PATH store, synthetic command resolver and subprocess runner, and disposable installation roots. It proves deterministic fast-path decisions, exact phase selection, interpreter/config identity holds, reparse/symlink refusal through the host analogue, fd-pinned receipt bounds, unpredictable exclusive staging, target-identity checks, registry-kind preservation at the mocked adapter seam, stale-PATH refusal, event-backed concurrent serialization, launcher bytes, idempotency, duplicate-entry repair, and explicit failures without touching a real user profile. The POSIX symlink-victim analogues are explicitly skipped on Windows, where production uses reparse-point and exact-handle checks; no test creates a privileged junction or mutates a live profile. The suite cannot prove Windows registry permissions or propagation, `py.exe` discovery, pip/network behavior, native junction behavior, Windows command resolution, WSL Windows-PATH import, mounted-file execution, or a real cross-boundary launch. Those facts remain the two live `NOT_RUN` rows above until exact-candidate evidence is recorded; no Linux run or synthetic win32 argument fills them.
