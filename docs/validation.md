...
| Synthetic machine-bootstrap regressions | The same complete suite under POSIX and simulated-Win32 providers: `.cmd` forms, case-insensitive PATH spelling, file-index-style identity, typed registry state, descriptor/share-binding analogues, pinned source-archive consumption, external receipt anchor, parent-bound venv/staging creation, concurrency, and fault seams; the thin native API wrapper remains Windows-only | No matching correction-candidate record yet | `NOT_RUN` |
