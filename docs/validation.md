...
| Synthetic machine-bootstrap regressions | Fake typed/transactional user-PATH store and resolver, disposable physically bound venv/install roots, descriptor-bound subprocess execution, handle-identity alias comparison, command-bound receipts, exclusive random staging with no-overwrite commits, event-backed concurrent invocations, synthetic subprocesses, and spelling-robust path faults; no live registry, PATH, or Python mutation | No matching correction-candidate record yet | `NOT_RUN` |
...
Unit coverage injects a memory-only typed user-PATH store, transactional registry adapter, synthetic command resolver and subprocess runner, and disposable installation roots. It proves deterministic command-bound fast-path decisions, exact phase selection, interpreter/config identity holds, descriptor-bound execution across a check/use substitution, reparse/symlink refusal through the host analogue, fd-pinned receipt bounds, spelling-independent handle identity, unpredictable exclusive staging, preservation of an interfering target, refusal without overwrite for mismatching launchers, registry-kind preservation and transaction-conflict behavior at the adapter seam, event-backed concurrent serialization, launcher bytes, idempotency, duplicate-entry repair, and explicit failures without touching a real user profile. The POSIX symlink-victim analogues are explicitly skipped on Windows, where production uses reparse-point handles retained without write/delete sharing; no test creates a privileged junction or mutates a live profile. The suite cannot prove Windows registry permissions, transaction-manager availability, environment propagation, `py.exe` discovery, pip/network behavior, native junction behavior, Windows command resolution, WSL Windows-PATH import, mounted-file execution, or a real cross-boundary launch. Those facts remain the two live `NOT_RUN` rows above until exact-candidate evidence is recorded; no Linux run or synthetic win32 argument fills them.
