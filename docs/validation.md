...
| Synthetic machine-bootstrap regressions | The same complete suite under POSIX and simulated-Win32 providers: `.cmd` forms, case-insensitive PATH spelling, file-index-style identity, typed registry state, target-parent byte-exact commits and EXDEV fallback, writer/read share admission, content-bound stage-to-binding and pin-to-effect handoffs, retained no-follow ancestry creation, randomized arbitrary-depth redirect denial, proof-preserving receipt diagnostics, concurrency, and fault seams; the native API wrapper remains Windows-only | No matching correction-candidate record yet | `NOT_RUN` |
...
Unit coverage injects a memory-only typed user-PATH store, transactional registry adapter, synthetic command resolver and subprocess runner, and disposable installation/source/state roots. A shared provider contract selects bounded reads, retained component descent, target-parent binary staging, exact commit verification, and effect redirect admission; native Windows and the simulated provider use different OS primitives at that boundary. The complete unit and incident corpus runs under both the POSIX provider and simulated-Win32 provider. The latter now models the symmetric Windows writer/reader share rule: a retained archive writer denies a strict reader, admits the write-sharing bridge, and then hands off to the strict pin without an identity gap. Regressions prove exact LF bytes, equal staged/committed digests, the same-parent `EXDEV` fallback, a content-and-change-token stage-to-binding predicate, and immutable pin-to-effect bytes that detect a changed-and-restored public inode without feeding those bytes to pip.

All bootstrap-owned ancestry creation and file writes descend from retained roots with no-follow at every component. The missing-ancestry race and seeded arbitrary-depth variants prove that replacing an earlier component cannot redirect a later creation; injected venv effects are denied when they attempt directory redirects at arbitrary depth, and the recursive post-effect scan rejects a remaining redirect. These are disposable provider tests, not live native-Windows evidence. Native CreateFile/CreateProcess behavior, real junctions, registry permissions, environment propagation, `py.exe` discovery, pip/network behavior, WSL Windows-PATH import, and mounted-file execution remain separate live `NOT_RUN` gates. The Linux-oriented non-native adapters still require verified `/proc/self/fd` for source, interpreter, and venv descriptor effects; archive snapshots accept `/proc/self/fd` or `/dev/fd`. Direct non-Windows setup remains `not_applicable`.
