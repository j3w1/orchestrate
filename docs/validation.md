...
| --- | --- | --- | --- |
| Fresh full suite | `python -m unittest discover -s tests -v` plus explicit `python -m unittest discover -s tests/incidents -t tests -v`; disposable Git and synthetic command fixtures only | No matching correction-candidate record yet | `NOT_RUN` |
...
| Synthetic machine-bootstrap regressions | Fake typed/transactional user-PATH store and resolver, disposable physically bound venv/install/source roots, descriptor-bound POSIX execution, Windows held-share/explicit-application-name boundaries, source- and command-bound tamper-evident receipts, handle-identity alias comparison, parent-handle-relative exclusive staging with no-overwrite commits, event-backed concurrent invocations, synthetic subprocesses, and spelling-robust path faults placed before platform opens/commits; no live registry, PATH, or Python mutation | No matching correction-candidate record yet | `NOT_RUN` |
...
Unit coverage injects a memory-only typed user-PATH store, transactional registry adapter, synthetic command resolver and subprocess runner, and disposable installation/source roots. It proves deterministic source- and command-bound fast-path decisions, first-position PATH enforcement, exact phase selection, interpreter/config identity holds, descriptor-bound POSIX execution across a check/use substitution, reviewed-root binding across editable-install substitution, source-change and receipt-tamper refusal, reparse/symlink refusal through the host analogue, descriptor-pinned receipt bounds through a pre-open fault seam shared by both platform branches, spelling-independent handle identity, parent-bound unpredictable exclusive staging, refusal before outside creation after parent substitution, preservation of an interfering target, refusal without overwrite for mismatching launchers, registry-kind preservation and transaction-conflict behavior at the adapter seam, event-backed concurrent serialization, launcher bytes, idempotency, duplicate-entry repair, and explicit failures without touching a real user profile. The Windows-only interpreter replacement test exercises native sharing semantics; POSIX substitution analogues exercise descriptor execution and parent/source binding host-neutrally. Production Windows uses actual-read handles without write/delete sharing plus explicit application-name execution and before/after identity verification, while POSIX executes through a descriptor. No test creates a privileged junction or mutates a live profile. The suite cannot prove Windows registry permissions, transaction-manager availability, environment propagation, `py.exe` discovery, pip/network behavior, native junction behavior, Windows command resolution, WSL Windows-PATH import, mounted-file execution, or a real cross-boundary launch. Those facts remain the two live `NOT_RUN` rows above until exact-candidate evidence is recorded; no Linux run or synthetic win32 argument fills them.
